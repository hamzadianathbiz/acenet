## OpenRouter connection persistence — 10 September 2026

Live Pages `c78d123a`, Worker `076fc17a-ea06-474b-87d2-3efb251cd1f3`.

Reproduced a callback containing `code` without ACENET's custom `or_state`: the old frontend ignored it, never exchanged the code and asked for connection again on the next message. The provider documents a code return; the user's exact callback URL was not observed. The frontend now detects code-only callbacks and recovers state from its tab marker or an authenticated tenant endpoint. The endpoint exposes only an unexpired state ID, never the verifier, key or draft. Server-side start reuses an existing encrypted connection instead of replacing the pending flow; identical successful finish retries do not exchange the one-time code again. Sending is guarded during connection completion. New chat also resets a Send button left disabled by the previous queued task.

47 focused Node tests pass. Actual-route browser integration passes locally and against canonical deployed assets: code-only callbacks with and without a tab marker, one connection followed by reload/follow-up/new-chat sends, stale browser status with an already-saved server key, private draft/files/chat restoration, one automatic send, settings without send, Back/decline/expiry, start errors and mobile layout. Browser account login and provider network are fixtures; this is not real OpenRouter authorization or model inference. Worker dry-run passes. Canonical HTML/JS/CSS hashes match; live callback recovery returns 401 anonymously and only the state field with no-store to an authenticated account. No helper upgrade or paid hosting change is required.
